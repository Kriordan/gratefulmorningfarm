# Grateful Morning Farm

## Overview
Grateful Morning Farm site
...
- `/gratefulmorningfarm` - the Django project folder
...
- Retrieve Media assets from AWS S3:
    - Set up and configure the AWS CLI: see <https://docs.aws.amazon.com/cli/latest/userguide/cli-chap-install.html> for instructions
        - be sure to set up your AWS config with your gratefulmorningfarm credentials: instructions here <https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-files.html>
    - make sure a directory exists with the path [project root]/gratefulmorningfarm/media
    - from the project root run: `aws s3 cp s3://gratefulmorningfarm-dev ./gratefulmorningfarm/media --recursive` replacing `gratefulmorningfarm-dev` with `gratefulmorningfarm-prod` or a different s3 bucket name as needed
